...
class BattleEyeClient {
    constructor(ip, port, password, messageTimeout = 3000) {
    ...
    sendCommand(command) {
        ...
        setTimeout(this.timeout, this.messageTimeout, this);
        this.send(packet);
    }
    send(data) {
        if (this.error)
    ...
    timeout(client) {
        if ((new Date().getTime() - client.lastResponse) >= this.messageTimeout) {
            try {
                client.close();
            } catch (error) {
            }
    ...
    stripHeaderServerMessage(message) {
    ...
    stripHeaderMultipacket(message) {
